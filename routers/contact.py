...
async def Add_contact(user_id: int, contactdata: ContactSchema.ContactInfoData,
    ...
    try:
        ...
        raise HTTPException(
            status_code=409, detail=str(e.orig))
    return response
...
@router.patch("/edit_contact/{contact_id}", response_model=ContactSchema.FinalContactInfoData)
# async def edit_contact(contactdata: ContactSchema.ContactInfoData,contact_id:int,db:Session = Depends(get_db)):
async def edit_address(contactdata: ContactSchema.ContactInfoData,
