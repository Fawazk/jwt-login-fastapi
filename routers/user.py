...
from operations import (
    user as functions,
    address as Address_functions,
    contact as Contact_functions,
)
...
async def get_users(
    ...
    current_user: UserSchema.FinalUserData = Depends(functions.get_current_active_user),
):
...
async def delete_user(
    ...
    response = functions.delete_user(db, current_user)
    return response


# @router.get("/user-all-data",response_model=userSchema.userAllData, tags=["user with all datas"])
@router.get("/user-all-data",response_model=UserSchema.UserAllData)
async def user_all_data(
    # user_id: int,
    db: Session = Depends(get_db),
    current_user: UserSchema.FinalUserData = Depends(functions.get_current_active_user),
):
    """This Api will show all the data's related to the user id which you passed"""
    userresponse = functions.get_users(db=db, user_id=current_user.id)
