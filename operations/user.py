...
async def get_current_active_user(current_user: UserSchema.FinalUserData = Depends(get_current_user)):
...
def delete_user(db, user_id):
    related_dbs = {}
    related_dbs['contact'] = db.query(contact_models.ContactInfo).filter(
        contact_models.ContactInfo.user_id == user_id).all()
    related_dbs['address'] = db.query(address_models.Addressess).filter(
        address_models.Addressess.user_id == user_id).all()
