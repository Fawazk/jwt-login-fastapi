...
def add_contact(db, contactdata: ContactSchema.ContactInfoData,user_id):
    ...
    if user:
        contactdata_db = ContactModels.ContactInfo(**contactdata.dict(),user_id=user_id)
        db.flush()
        ...
        raise HTTPException(
            status_code=404, detail="This user id is not found in our database . pls enter any correct user id")
...
def get_contact(db,user_id):
    user = db.get(UserModels.User, user_id)
    if user:
        list_of_contact = db.query(ContactModels.ContactInfo).filter(ContactModels.ContactInfo.user_id == user_id).all()
        if list_of_contact:
            return list_of_contact
        else:
            return [{'detail':"This user have no contact data in our database , pls add contact"}]
    else:
        raise HTTPException(
            status_code=404, detail="This user id is not found in our database . pls enter any correct user id")
...
def edit_contact(db,contactdata,contact_id):
    ...
    if contact:
        ...
        raise HTTPException(
            status_code=404, detail="This contact id is not found in our database . pls enter any correct contact id")
