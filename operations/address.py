...
def get_address(db,user_id):
    ...
    if user:
        list_of_address = db.query(models.Addressess).filter(models.Addressess.user_id == user_id).all()
        if list_of_address:
            return list_of_address
        else:
            return [{"detail":"This user have no address data in our database , pls add address"}]
    else:
        raise HTTPException(
